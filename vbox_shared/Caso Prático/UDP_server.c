...
void* handle_client_request_master(void *arg){
    while(1){
        ...
        char* do1 = (char*)calloc(strlen(items->dateObserved) + 1, sizeof(char));
        char* do2 = (char*)calloc(strlen((items+1)->dateObserved) + 1, sizeof(char));

        strcpy(do1, items->dateObserved);
        strcpy(do2, (items+1)->dateObserved);
        ...
        timestamp_t ts1;
        decode_date_observed(do1, &ts1);

        timestamp_t ts2;
        decode_date_observed(do2, &ts2);
        ...
        for(int i=0;i<=size/2;i++){
            ...
            for(int j=i+1;j<size;j++){
                long diff;
                if((diff = calculate_time_difference(ts1, ts2)) > MARGIN_ERROR_MS){
